sap.ui.define([
    ...
    return Controller.extend("kyra001.pages.access.AccessPage", {
        onInit() {
            ...
            const oModel = new JSONModel({
                ...
                adminPersonaUsers: [
                    ...
                    { userId: "app01", currentPersona: "Approver", targetPersona: "Compliance Reviewer", status: "Active" },
                    { userId: "comp01", currentPersona: "Compliance Reviewer", targetPersona: "Approver", status: "Active" }
                ],
                personaLookupInput: "",
                ...
                personaLookupUser: {
                    ...
                    email: "",
                    department: "",
                    currentPersona: "Requester",
                    selectedPersona: "Requester",
                    targetPersona: "Requester",
                    sourceTable: "access_management.requester_id",
                    status: "Active",
                    entitlementsCount: 0,
                    entitlements: []
                }
        ...
        onExit() {
            ...
            if (this._fnNotifOutsideClickListener) {
                document.removeEventListener("pointerdown", this._fnNotifOutsideClickListener, true);
                this._fnNotifOutsideClickListener = null;
            }
        ...
        _onRouteMatched() {
            ...
            const cleanTeamStr = (s) => {
                if (!s) return "";
                let str = String(s).trim();
                str = str.replace(/\s*\([^)]*\)\s*$/g, "").trim();
                str = str.replace(/\s+team$/i, "").trim();
                return str;
            };

            const sRaw = (oItem.team || oItem.business_function || oItem.businessFunction || oItem.function || oItem.service_topic || oItem.serviceTopic || oItem.category || oItem.role_name || oItem.roleName || "").toString().trim();
            const sLower = sRaw.toLowerCase();
            
            if (sLower.includes("procure") || sLower.includes("purchas") || sLower.includes("buyer") || sLower.includes("vendor") || sLower.includes("sourcing") || sLower.includes("ariba")) {
                return "Procurement";
            }
            if (sLower.includes("finan") || sLower.includes("account") || sLower.includes("treasur") || sLower.includes("tax") || sLower.includes("ledger") || sLower.includes("controlling") || sLower.includes("fico") || sLower.includes("s/4hana")) {
                return "Finance";
            }
            if (sLower.includes("audit") || sLower.includes("complian") || sLower.includes("risk") || sLower.includes("governan") || sLower.includes("isrm") || sLower.includes("grc")) {
                return "Audit & Compliance";
            }
            if (sLower.includes("secur") || sLower.includes("iam") || sLower.includes("identity") || sLower.includes("cyber") || sLower.includes("access")) {
                return "Security & IAM";
            }
            if (sLower.includes("logic") || sLower.includes("warehous") || sLower.includes("supply") || sLower.includes("inventor") || sLower.includes("plant") || sLower.includes("mfg") || sLower.includes("manufactur") || sLower.includes("shipping")) {
                return "Supply Chain & Logistics";
            }
            if (sLower.includes("sale") || sLower.includes("order") || sLower.includes("distribut") || sLower.includes("market") || sLower.includes("customer") || sLower.includes("crm")) {
                return "Sales & Distribution";
            }
            if (sLower.includes("hr") || sLower.includes("people") || sLower.includes("workforce") || sLower.includes("payroll") || sLower.includes("talent") || sLower.includes("successfactors")) {
                return "HR & People Operations";
            }
            if (sLower.includes("develop") || sLower.includes("engineer") || sLower.includes("tech") || sLower.includes("ui5") || sLower.includes("btp") || sLower.includes("cloud") || sLower.includes("admin") || sLower.includes("infra")) {
                return "Engineering & Cloud Ops";
            }
            if (oItem.team && !oItem.team.toLowerCase().includes("category") && !oItem.team.toLowerCase().includes("revocation")) {
                return cleanTeamStr(oItem.team);
            }
            return cleanTeamStr(sRaw) || "Enterprise Operations";
        },
        ...
        _buildApproverHistoryAndPending(aRawRecords) {
            ...
            aApproverPending.sort((a, b) => {
                const tA = getPendingTimeAccess(a);
                const tB = getPendingTimeAccess(b);
                if (tA !== tB && tA > 0 && tB > 0) return tA - tB;
                return (a.requestId || "").localeCompare(b.requestId || "");
            });
            return {
                pending: aApproverPending,
        ...
        async _loadSubmittedRequests(oModel, bSilent = false, bForce = false) {
            ...
            if (!aRawDbRequests || aRawDbRequests.length === 0) {
                this._localInFlightRevocations = {};
                try {
                    sessionStorage.removeItem("kyra_pending_revocations");
                    sessionStorage.removeItem("kyra_deleted_entitlements");
                    sessionStorage.removeItem("kyra_deleted_requests");
                } catch(e) {}
                this._setSmartProperty(oModel, "/userAccessList", []);
                this._setSmartProperty(oModel, "/activeRoles", []);
                ...
                this._setSmartProperty(oModel, "/pendingRevokeCount", 0);
                this._setSmartProperty(oModel, "/processedRequests", []);
                this._setSmartProperty(oModel, "/processedAccessRequests", []);
                this._setSmartProperty(oModel, "/processedRevokeRequests", []);
                this._setSmartProperty(oModel, "/allSubmittedRequests", []);
            ...
            aRawDbRequests.forEach(r => {
                ...
                const sGrantedDateForExp = r.granted_date || (r.updated_at ? r.updated_at.split("T")[0] : null) || (r.created_at ? r.created_at.split("T")[0] : null) || "";
                const oExpCalc = calculateExpiryDays(r.access_duration, sGrantedDateForExp);
                const isExpiredReq = !isRevocationReq && (
                    sDbStatus === "EXPIRED" ||
                    (r.access_type || "").toUpperCase() === "EXPIRED" ||
                    (isOverallApproved && oExpCalc.isExpired === true)
                );
                ...
                if (!oGrouped[sGroupKey]) {
                    oGrouped[sGroupKey] = {
                        ...
                        sector: r.business_sector || "",
                        function: r.business_function || "Corporate Governance",
                        businessFunction: r.business_function || "Corporate Governance",
                        region: r.operating_region || "",
                        justification: r.justification || "",
            ...
            Object.keys(roleStates).forEach(sKey => {
                ...
                const sService = deriveServiceTopicFromRole(r.role_name, r.service_topic);
            ...
            this._setSmartProperty(oModel, "/requestHistory", aDisplayedHistory);
            this._setSmartProperty(oModel, "/myHistoryRequests", aDisplayedHistory);
            ...
            this._setSmartProperty(oModel, "/filteredHistoryCount", aDisplayedHistory.length);
            ...
            // Merge freshly processed decisions from sessionStorage so instant UI response is preserved
            try {
        ...
        _loadNotifications(oModel, aExplicitDbRequests) {
            ...
            aUserRequests.forEach(r => {
                ...
                // 2. Notification: Line Manager / Initial Approver Stage
                ...
                // 3. Notification: Compliance Reviewer Stage (only for conflict requests)
                const sCompStatus = (r.compliance_status || r.compliance_decision_status || "").toUpperCase();
                if (sCompStatus === "APPROVED") {
                    ...
                    if (!oDeletedSet.has(sCompId) && !oAddedNotifIds.has(sCompId)) {
                        ...
                        aNotifications.push({
                            ...
                            timestamp: formatTimeAgo(r.compliance_decision_created_at || r.compliance_created_at || r.updated_at || sCreatedTime),
                            rawTimestamp: parseRawTimestamp(r.compliance_decision_created_at || r.compliance_created_at || r.updated_at || sCreatedTime),
                            stagePriority: 3,
                            icon: "sap-icon://sys-enter-2",
                            state: "Success",
                ...
                // 4. Notification: IAM Approver 1 Stage
                const sIam1Status = (r.iam_approver_1_status || r.iam_approver_1_decision_status || "").toUpperCase();
                if (sIam1Status === "APPROVED") {
                    ...
                    if (!oDeletedSet.has(sIam1Id) && !oAddedNotifIds.has(sIam1Id)) {
                        ...
                        aNotifications.push({
                            ...
                            type: "approved",
                            category: "Access Decisions",
                            title: "IAM Approver 1 Stage Approved",
                            ...
                            timestamp: formatTimeAgo(r.iam_approver_1_decision_created_at || r.iam_approver_1_created_at || r.updated_at || sCreatedTime),
                            rawTimestamp: parseRawTimestamp(r.iam_approver_1_decision_created_at || r.iam_approver_1_created_at || r.updated_at || sCreatedTime),
                            stagePriority: 4,
                            icon: "sap-icon://sys-enter-2",
                            state: "Success",
                ...
                // 5. Notification: IAM Approver 2 / Final Stage
                const sIam2Status = (r.iam_approver_2_status || r.iam_approver_2_decision_status || "").toUpperCase();
                ...
                if (sIam2Status === "APPROVED" || sDbStatus === "APPROVED") {
                    ...
                    if (!oDeletedSet.has(sIam2Id) && !oAddedNotifIds.has(sIam2Id)) {
                        ...
                        aNotifications.push({
                            ...
                            timestamp: formatTimeAgo(r.iam_approver_2_decision_created_at || r.iam_approver_2_created_at || r.updated_at || sCreatedTime),
                            rawTimestamp: parseRawTimestamp(r.iam_approver_2_decision_created_at || r.iam_approver_2_created_at || r.updated_at || sCreatedTime),
                            stagePriority: 5,
                            icon: "sap-icon://sys-enter-2",
                            state: "Success",
            ...
            // Sort notifications strictly in REVERSE CHRONOLOGICAL ORDER (newest first)
            const sortNotificationsDesc = (a, b) => {
                const tA = Number(a.rawTimestamp || 0);
                const tB = Number(b.rawTimestamp || 0);
                // If timestamps differ by more than 2 seconds, strictly sort newest first
                if (Math.abs(tB - tA) > 2000) {
                    return tB - tA;
                }
                // When decisions occur at virtually the same time, later stage always appears above earlier stage
            ...
            aNotifications.sort(sortNotificationsDesc);
                ...
                if (aTopNotifs.length === 0) {
                    ...
                    aTopNotifs.forEach((n, idx) => {
                        ...
                        if (n.type === "approved") {
                            ...
                            `;
                        } else if (n.type === "revocation" || n.isRevocation) {
                            sAvatarHtml = `
                                <div class="kyra-notif-avatar-circle" style="background: #FFF7ED; border: 1px solid #FFEDD5;">
                            ...
                            `;
                        } else if (n.type === "access_review") {
                            sAvatarHtml = `
                                <div class="kyra-notif-avatar-circle" style="background: #EFF6FF; border: 1px solid #DBEAFE;">
        ...
        _updateSubRolesList(bPreserveSelections) {
            ...
            const aActiveServices = (oModel.getProperty("/adminServicesAll") || [])
                .filter(s => s && s.status !== "Inactive")
                .map(s => s.serviceName);

            const oServicesRolesMap = {};
            Object.keys(oDetailsMap).forEach(sServiceKey => {
                if (aActiveServices.length > 0 && !aActiveServices.includes(sServiceKey)) {
                    return;
                }
                const aTeams = oDetailsMap[sServiceKey] || [];
                oServicesRolesMap[sServiceKey] = aTeams
                    .filter(t => t && t.name && t.status !== "Inactive")
                    .map(t => ({
                        key: t.name.trim(),
                        text: t.name.trim(),
            ...
            let aAllSubRoles = [];
            Object.keys(oServicesRolesMap).forEach(sServiceKey => {
                aAllSubRoles = aAllSubRoles.concat(oServicesRolesMap[sServiceKey] || []);
            });
        ...
        _updatePersonasList(bPreserveSelections) {
            ...
            Object.keys(oDetailsMap).forEach(sServiceKey => {
                ...
                aTeams.forEach(oTeam => {
                    if (!oTeam || !oTeam.name || oTeam.status === "Inactive") return;
                    const sTeamName = oTeam.name.trim();
                    const sCleanTeamShort = sTeamName.replace(/\s*\([^)]*\)/g, "").trim();
                    const aActivePersonas = (oTeam.subClassifications || [])
                        .filter(p => p && p.name && p.status !== "Inactive")
                        .map(p => {
                            let sPersName = p.name.trim();
            ...
            aSelectedRoles.forEach(sRoleKey => {
                if (oTeamPersonasMap[sRoleKey]) {
                    ...
                    const sRoleShort = sRoleKey.replace(/\s*\([^)]*\)/g, "").trim().toLowerCase();
                    const sMatchedKey = Object.keys(oTeamPersonasMap).find(k =>
                        k.replace(/\s*\([^)]*\)/g, "").trim().toLowerCase() === sRoleShort
                    );
                    if (sMatchedKey && oTeamPersonasMap[sMatchedKey]) {
                        aCombinedPersonas = aCombinedPersonas.concat(oTeamPersonasMap[sMatchedKey]);
                    }
                }
            });
        ...
        onGoToAddAccessStep4() {
            ...
            // Only save system slide config if currently on SubStep 1
            if (oModel.getProperty("/addAccessConfigSubStep") !== 2) {
                this._saveCurrentSystemSlideConfig();
            }

            let sRegion = oModel.getProperty("/addAccessRegion");
            let aSystems = oModel.getProperty("/addAccessSelectedSystems") || [];
            let sDuration = oModel.getProperty("/addAccessDuration");
            let sJustification = (oModel.getProperty("/addAccessJustification") || "").trim();

            // Read justification from textarea DOM/control if model hasn't synced yet
            if (!sJustification) {
                const oArea = this.byId("inPageJustificationArea");
                if (oArea && typeof oArea.getValue === "function") {
                    sJustification = (oArea.getValue() || "").trim();
                    if (sJustification) {
            ...
            if (!sRegion || sRegion.trim() === "") {
                const aMapRegions = oModel.getProperty("/mapSelectedRegions") || [];
                sRegion = aMapRegions.length > 0 ? aMapRegions.map(r => r.name || r).join(", ") : "Asia";
                oModel.setProperty("/addAccessRegion", sRegion);
            }
            if (!aSystems || aSystems.length === 0) {
                aSystems = ["SAP S/4HANA Enterprise"];
            ...
            const oLiveDetailsMap = oModel.getProperty("/adminServiceDetailsMap") || {};
            Object.keys(oLiveDetailsMap).forEach(sServiceKey => {
                const aTeams = oLiveDetailsMap[sServiceKey] || [];
                aTeams.forEach(oTeam => {
                    if (!oTeam || !oTeam.name || oTeam.status === "Inactive") return;
                    const sTeamName = oTeam.name.trim();
                    const sCleanTeamShort = sTeamName.replace(/\s*\([^)]*\)/g, "").trim();
                    oRoleToPersonas[sTeamName] = (oTeam.subClassifications || [])
                        .filter(p => p && p.name && p.status !== "Inactive")
                        .map(p => {
                            let sPersName = p.name.trim();
                            if (!sPersName.includes("(") && sCleanTeamShort) {
            ...
            const cleanPersonaName = (s) => {
                ...
                let str = String(s).trim();
                str = str.replace(/\s*\([^)]*\)\s*$/g, "").trim();
                str = str.replace(/\s+persona\b/gi, "").trim();
                return str || s;
            };
            ...
            aSystems.forEach((sSys) => {
                ...
                let aSysServices = oSysConfig.selectedServices || [];
                let aSysRoles = oSysConfig.selectedRoles || [];
                let aSysPersonas = oSysConfig.selectedPersonas || [];

                // Auto-derive personas from roles if personas were not explicitly chosen
                if ((!aSysPersonas || aSysPersonas.length === 0) && aSysRoles && aSysRoles.length > 0) {
                    aSysPersonas = [];
                    aSysRoles.forEach(r => {
                        if (oRoleToPersonas[r] && oRoleToPersonas[r].length > 0) {
                            aSysPersonas.push(oRoleToPersonas[r][0]);
                        } else {
                            aSysPersonas.push(r.replace(/\s*\(.*?\)/, "") + " Persona");
                        }
                    });
                }
                ...
                const processItem = (sRole, sPers) => {
                    ...
                    // Check 1: Active Assigned Access in user's profile
                    ...
                    const bAlreadyActive = aActiveList.some(ar => {
                        ...
                        const isActive = sStat.includes("active") || sStat.includes("approved") || sStat === "success" || !sStat;
                        return isActive && isMatch(ar, oItem);
                    });

                    // Check 2: Pending request in approval queue
                    ...
                    const aAllUserAcc = oModel.getProperty("/userAccessList") || [];
                    const aPendingRevList = (oModel.getProperty("/myPendingRequests") || oModel.getProperty("/pendingRevokeRequests") || [])
                        .filter(p => p.type === "Revocation" || p.type === "Revoke" || p.isRevocation === true || (p.status || "").toLowerCase().includes("revoke") || (p.requestType || "").toLowerCase().includes("revoke"));
                    const aInflightRevList = this._localInFlightRevocations ? Object.values(this._localInFlightRevocations) : [];
                    let aStoredRevList = [];
                    try { aStoredRevList = JSON.parse(sessionStorage.getItem("kyra_pending_revocations") || "[]"); } catch(e) {}

                    const bRevokePending = aAllUserAcc.some(ar => {
                        const sStat = (ar.status || "").toLowerCase();
                        const isRev = sStat.includes("revoke") || ar.isRevocation;
                        return isRev && isMatch(ar, oItem);
                    }) || aPendingRevList.some(pr => {
                        if (isMatch(pr, oItem)) return true;
                        if (Array.isArray(pr.entitlements)) {
                            return pr.entitlements.some(e => isMatch(e, oItem));
                        }
                        return false;
                    }) || aInflightRevList.some(ir => isMatch(ir, oItem))
                       || aStoredRevList.some(sr => isMatch(sr, oItem));

                    // Status assignment: No 'Duplicate in Request'; check Revoke Pending then Already Active
                    if (bRevokePending) {
                        oItem.existingStatus = "Revoke in Pending";
                        oItem.existingState = "Warning";
                        oItem.existingIcon = "sap-icon://pending";
                        oItem.statusType = "revoke_pending";
            ...
            const aRestrictedRecords = aSummaryItems
                ...
                return {
                    ...
                    systemName: sSys,
                    services: sServices,
                    serviceTopic: sServices,
                    moduleName: sServices,
                    team: sTeam,
                    teamName: sTeam,
                    roleTitle: sTeam,
                    roleName: sTeam,
                    functionalRole: sTeam,
                    persona: sPersona,
                    selectedPersona: sPersona,
                    securityGroup: sSecGroup,
                    selectedSecurityGroups: sSecGroup,
                    adGroup: sAdGroup,
            ...
            try {
                ...
                console.warn("SoD Conflict evaluation error:", e);
            }
            ...
            try {
                ...
                console.warn("Threshold evaluation error:", e);
            }
        ...
        _evaluateThresholdAndDuplicates(aSummaryItems) {
            ...
            const aPendingRequests = (oModel.getProperty("/myPendingRequests") || oModel.getProperty("/pendingAccessRequests") || [])
                .filter(p => {
                    ...
                    const isRevoke = p.type === "Revocation" || p.type === "Revoke" || p.isRevocation === true || sStatus.includes("revoke");
            ...
            const aAllUserAccessList = oModel.getProperty("/userAccessList") || [];
            const aPendingRevokesList = (oModel.getProperty("/myPendingRequests") || oModel.getProperty("/pendingRevokeRequests") || [])
                .filter(p => p.type === "Revocation" || p.type === "Revoke" || p.isRevocation === true || (p.status || "").toLowerCase().includes("revoke") || (p.requestType || "").toLowerCase().includes("revoke") || (p.accessType || "").toLowerCase().includes("revoke"));
            const aInflightRevokesList = this._localInFlightRevocations ? Object.values(this._localInFlightRevocations) : [];
            let aStoredRevokesList = [];
            try { aStoredRevokesList = JSON.parse(sessionStorage.getItem("kyra_pending_revocations") || "[]"); } catch(e) {}
            ...
            aSummaryItems.forEach((item) => {
                ...
                // Check 0: Revoke in Pending (submitted revoke request for active access currently pending)
                const bFoundInRevokePending = aAllUserAccessList.some(ar => {
                    const sStat = (ar.status || "").toLowerCase();
                    const isRev = sStat.includes("revoke") || ar.isRevocation;
                    return isRev && isMatch(ar, item);
                }) || aPendingRevokesList.some(pr => {
                    if (isMatch(pr, item)) return true;
                    if (Array.isArray(pr.entitlements)) {
                        return pr.entitlements.some(e => isMatch(e, item));
                    }
                    return false;
                }) || aInflightRevokesList.some(ir => isMatch(ir, item))
                   || aStoredRevokesList.some(sr => isMatch(sr, item));

                // Check 2: Active Assigned Access in My Access Section (already active and not revoking)
                const bFoundInActive = aActiveAccess.some(ar => {
                    const sStat = (ar.status || "").toLowerCase();
                    const isRev = sStat.includes("revoke") || ar.isRevocation;
                    return !isRev && isMatch(ar, item);
                });
                ...
                if (bFoundInRevokePending) {
                    aDuplicateRoles.push({
                        ...
                        roleName: sTeam,
                        functionalRole: sTeam,
                        persona: sPersonaClean,
                        selectedPersona: sPersonaClean,
                        securityGroup: sSecGrp,
                        selectedSecurityGroups: sSecGrp,
                        adGroup: sAdGrp,
                        adGroupName: sAdGrp,
        ...
        async onFinalSubmitInPageAddAccess() {
            ...
            // Filter out items that are already pending, already active, or revoke in pending
            ...
            const aValidItems = aSummaryItems.filter(item => !isExcludedFromSubmit(item));
            const aSkippedItems = aSummaryItems.filter(item => isExcludedFromSubmit(item));

            if (aValidItems.length === 0) {
                MessageBox.warning("Configured access entitlement(s) with 'Revoke in Pending' or existing access status cannot be submitted. All items have been excluded.");
                return;
            }
            ...
            if (aSkippedItems.length > 0) {
                sPopupHtml += `
                    ...
                    <div style="margin-bottom: 4px;">
                        ${aSkippedItems.map(i => {
                            const sCleanRole = (i.roleTitle || i.roleName || 'Access Role').replace(/\s*\([^)]*\)/g, "");
                            const isRevokePending = (i.statusType === "revoke_pending" || (i.existingStatus && i.existingStatus.toLowerCase().includes("revoke")));
        ...
        onCloseApprovedSection() {
            ...
            let oItem = null;
            if (oEvent && (oEvent.requestId || oEvent.requestNumber)) {
                oItem = oEvent;
            } else if (oEvent && oEvent.getSource) {
                const oContext = oEvent.getSource().getBindingContext("accessModel");
                if (oContext) {
        ...
        _buildRequestDetailFromItem(oItem, oDbItem) {
            ...
            return {
                ...
                persona: sPersona,
                selectedPersona: sPersona,
                submittedDate: sSubmittedDateStr,
        ...
        onFilterHistoryByAll() {
            ...
            this._lastHistorySelectionState = null;
            const oModel = this.getView().getModel("accessModel");
            if (oModel) {
        ...
        onFilterHistoryByExpired() {
            const isMatch = (item) => {
                ...
                return item.isExpired === true || sStat.includes("expir") || sType.includes("expir");
            };
            ...
            if (oModel) {
                ...
                oModel.setProperty("/showHistorySection", true);
                oModel.setProperty("/activeKpiFilter", "ALL");
                oModel.setProperty("/historyFilterTitle", "Expired History");
                oModel.setProperty("/historyFilterSubtitle", "Filtered by: Expired");
            ...
            sap.ui.require(["sap/m/MessageToast"], (MessageToast) => {
                MessageToast.show("Filtered by Expired access requests.");
            });
        },
        ...
        onFilterHistoryByRevoked() {
            const isMatch = (item) => {
                ...
                return !isExp && (sStat === "revoked" || (isRevoc && (sStat.includes("approved") || sStat.includes("active") || sStat.includes("success")))) && !sStat.includes("pending") && !sStat.includes("reject");
            };
            this._currentHistoryFilterMatchesItem = isMatch;
            this._lastHistorySelectionState = null;
            const oModel = this.getView().getModel("accessModel");
        ...
        onFilterHistoryByApproved() {
            const isMatch = (item) => {
                ...
                const isExp = item.isExpired === true || sStat.includes("expir") || sType.includes("expir");
                const isRevoc = sType.includes("revok") || (item.function || "").toLowerCase().includes("revocation") || (item.requestId || "").toUpperCase().startsWith("REV-");
                return (sStat.includes("approved") || sStat.includes("active")) && !isRevoc && !isExp && !sStat.includes("pending") && !sStat.includes("reject");
            };
            this._currentHistoryFilterMatchesItem = isMatch;
            this._lastHistorySelectionState = null;
            const oModel = this.getView().getModel("accessModel");
        ...
        onFilterHistoryByRejected() {
            const isMatch = (item) => {
                ...
                const sType = (item.type || "").toLowerCase();
                const isExp = item.isExpired === true || sStat.includes("expir") || sType.includes("expir");
                return (sStat.includes("reject") || sStat.includes("decline")) && !isExp;
            };
            this._currentHistoryFilterMatchesItem = isMatch;
            this._lastHistorySelectionState = null;
            const oModel = this.getView().getModel("accessModel");
        ...
        onFilterHistoryDialog() {
            ...
            sap.ui.require([
                ...
                // Multiple selection state (preserve previous selection in same KPI tab if active)
                const oSelectionState = (this._lastHistorySelectionState && this._lastHistorySelectionKpi === sActiveKpi)
                    ? Object.assign({}, this._lastHistorySelectionState)
                    : {
                        ALL: true,
                        ADDITION: false,
                        REVOKE: false,
                ...
                const applySelectedFilter = () => {
                    ...
                    const matchesItem = (item) => {
                        ...
                        if (bShowRequestTypeFilter && !oSelectionState.ALL) {
                            ...
                            if (bHasTypeFilter) {
                                ...
                                const bMatchRevoke = oSelectionState.REVOKE && isRevoc && !isExp;
                    ...
                    this._lastHistorySelectionState = Object.assign({}, oSelectionState);
                    this._lastHistorySelectionKpi = sActiveKpi;

                    if (oSelectionState.ALL && !oSelectionState.CUSTOM) {
                        this._currentHistoryFilterMatchesItem = bIsAllHistory ? null : matchesItem;
        ...
        _performLogout() {
            ...
            if (oAccessModel) {
                ...
                oAccessModel.setProperty("/processedAccessRequests", []);
                oAccessModel.setProperty("/processedRevokeRequests", []);
        ...
        _normalizeRoleToPersonaName(sRole) {
            if (!sRole) return "";
        ...
        onToggleAdminSystemStatus(oEvent) {
            ...
            const aAll = (oModel.getProperty("/adminSystemsAll") || []).map(item =>
                item.systemName === oObj.systemName ? Object.assign({}, item, { status: sNextStatus }) : item
            );
            oModel.setProperty("/adminSystemsAll", aAll);
            oModel.setProperty("/adminSystems", aAll.slice());
            this._syncAdminConfigToLiveAddAccess(oModel);
        ...
        onSaveCustomConflictDraft() {
            ...
            if (sRole1 === sRole2) {
                MessageToast.show("Primary and Conflicting Persona must be different.");
                return;
            }
        ...
        onToggleAdminConflictStatus(oEvent) {
            ...
            if (!oModel || !oCtx) return;
            const oRule = oCtx.getObject();
            const sNextStatus = oRule.status === "Inactive" ? "Active" : "Inactive";

            const aAll = (oModel.getProperty("/adminCustomConflictsAll") || []).map(item => {
                if (item.role1 === oRule.role1 && item.role2 === oRule.role2 && item.system === oRule.system) {
                    return Object.assign({}, item, { status: sNextStatus });
                }
                return item;
            });
            oModel.setProperty("/adminCustomConflictsAll", aAll);
        ...
        onToggleAdminPersonaUserStatus(oEvent) {
            ...
            oModel.setProperty(oCtx.getPath() + "/status", sNextStatus);
            this._syncAdminConfigToLiveAddAccess(oModel);
            MessageToast.show("User '" + oObj.userId + "' persona status is now " + sNextStatus + ".");
        },

        onSaveAdminPersonaConversion() {
            const oModel = this.getView().getModel("accessModel");
            if (!oModel) return;
            this._syncAdminConfigToLiveAddAccess(oModel);
            MessageToast.show("User Persona Conversion saved and activated.");
        },
        ...
        onConvertUserPersona(oEvent) {
            ...
            const sNextPersona = window.prompt("Convert User '" + oUser.userId + "' to Persona (Requester / Approver / Compliance Reviewer):", oUser.targetPersona || "Approver");
            if (sNextPersona && sNextPersona.trim()) {
                oModel.setProperty(oCtx.getPath() + "/currentPersona", sNextPersona.trim());
                oModel.setProperty(oCtx.getPath() + "/status", "Active");
                this._syncAdminConfigToLiveAddAccess(oModel);
                MessageToast.show("User '" + oUser.userId + "' converted to " + sNextPersona.trim() + " and activated.");
            }
        },
        ...
        async onLookupEmployeePersona() {
            const oModel = this.getView().getModel("accessModel");
            if (!oModel) return;
            const oInputCtrl = this.byId("adminPersonaLookupInput");
            ...
            try {
                const res = await fetch("/odata/v4/admin-portal/lookupUserPersona", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                ...
                if (oData && (oData.ok || oData.found)) {
                    ...
                    } else if (typeof oUser.entitlementsJson === "string" && oUser.entitlementsJson) {
            ...
            let sFallbackName = "Enterprise Employee (" + sUsername + ")";
            let sFallbackDept = "Enterprise Business Operations";
            if (sLower.startsWith("app")) {
                sFallbackPersona = "Approver";
                sFallbackTable = "access_management.approver_id";
                sFallbackName = "Line Manager / Approver (" + sUsername + ")";
                sFallbackDept = "IT Governance & Approvals";
            } else if (sLower.startsWith("comp")) {
                sFallbackPersona = "Compliance Reviewer";
                sFallbackTable = "access_management.compliance_reviewer_id";
                sFallbackName = "Compliance Officer (" + sUsername + ")";
                sFallbackDept = "Risk & Compliance Governance";
            } else if (sLower === "emp001") {
                sFallbackName = "John Doe";
                sFallbackDept = "Engineering & Cloud Operations";
            }
            oModel.setProperty("/personaLookupUser", {
                userId: sLower,
                username: sLower,
        ...
        onToggleLookupUserStatus() {
            ...
            const sCurrent = oModel.getProperty("/personaLookupUser/status") || "Active";
            const sNext = sCurrent === "Inactive" ? "Active" : "Inactive";
            oModel.setProperty("/personaLookupUser/status", sNext);
        ...
        async onSaveConvertedUserPersona() {
            const oModel = this.getView().getModel("accessModel");
            if (!oModel || !oModel.getProperty("/personaLookupUserFound")) return;
            const oUser = oModel.getProperty("/personaLookupUser") || {};
            const oSelectCtrl = this.byId("adminUserPersonaSelect");
            const sSelectKey = (oSelectCtrl && typeof oSelectCtrl.getSelectedKey === "function") ? oSelectCtrl.getSelectedKey() : "";
            ...
            try {
                const res = await fetch("/odata/v4/admin-portal/convertUserPersona", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                        username: sUsername,
                ...
                oModel.setProperty("/personaLookupUser/userId", sUsername);
                oModel.setProperty("/personaLookupUser/username", sUsername);
                oModel.setProperty("/personaLookupUser/currentPersona", sTargetPersona);
                ...
                const iIdx = aPersonaUsers.findIndex(u => (u.userId || "").toLowerCase() === sUsername.toLowerCase());
                if (iIdx >= 0) {
