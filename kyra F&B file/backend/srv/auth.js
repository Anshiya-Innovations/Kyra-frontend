...
import { syncEvents, incrementDbVersion } from '../server.js';
...
export default async function() {
  ...
  // Enforce strict ascending order (created_at ASC) for all database reads
  this.before('READ', 'Requests', (req) => {
    if (req.query && req.query.SELECT && !req.query.SELECT.orderBy) {
      req.query.SELECT.orderBy = [{ ref: ['created_at'], sort: 'asc' }];
    }
  });
  ...
  this.on('submitAccessRequest', async (req) => {
    ...
    // Role normalization function to match conflicts (ignoring "EDAL" prefix and case)
    const normalizeRole = (s) => {
      if (!s) return "";
      let res = s.trim().toLowerCase();
      if (res.startsWith("edal ")) {
        res = res.substring(5).trim();
      }
      if (res === "it developer" || res === "it developers" || res === "developer") return "it developers";
      if (res === "it admin" || res === "it administrators" || res === "it administrator" || res === "admin") return "it administrators";
      if (res === "it security" || res === "security") return "it security";
      if (res === "compliance manager" || res === "compliance") return "compliance manager";
      return res;
    };

    const CONFLICT_MAP = {
      "it developers": ["it administrators", "it security"],
      "it administrators": ["it developers", "compliance manager", "it security"],
      "it security": ["it developers", "it administrators", "compliance manager"],
      "compliance manager": ["it administrators", "it security"]
    };

    const additions = requests.filter(r => {
      const type = (r.accessType || r.type || "").toUpperCase();
      const func = (r.businessFunction || "").toUpperCase();
      return type !== 'REVOCATION' && func !== 'ACCESS REVOCATION';
    });

    if (additions.length > 0) {
      const sActiveUser = requests[0].requesterUsername || 'Dev001';
      
      const userActiveRes = await db.run(
        `SELECT role_name, selected_persona FROM access_management.requests 
         WHERE requester_username = $1 AND status IN ('APPROVED', 'PENDING') 
         AND access_type != 'REVOCATION' AND business_function != 'Access Revocation'`,
        [sActiveUser]
      );
      const existingRoles = (userActiveRes || []).map(r => r.role_name);
      const existingPersonas = (userActiveRes || []).map(r => r.selected_persona || "");

      // Conflict checks bypassed per user configuration
      // (Role and Persona SoD checks temporarily disabled)
    }
  ...
  this.on('submitAccessDecision', async (req) => {
    ...
    for (const d of decisions) {
      ...
      if (requestRow) {
        // 3. Log detailed approval decision with ALL fields into approvals table
        const approvalId = cds.utils.uuid();
        const sApproverComment = d.comments || d.comment || `Entitlement ${sStatus} by Approver`;
        const sApproverUser = (req.user && req.user.id !== 'anonymous') ? req.user.id : 'Stake001';

        await db.run(
          `INSERT INTO access_management.approvals (
            id, request_number, requester_username, requester_persona,
            business_sector, business_function, operating_region,
            target_system, role_name, selected_persona,
            access_duration, justification, approver_username,
            approver_persona, status, comments, created_at
          ) VALUES (
            $1::uuid, $2, $3, $4,
            $5, $6, $7,
            $8, $9, $10,
          ...
          [
            approvalId,
            sReqNum,
            requestRow.requester_username,
            requestRow.requester_persona,
            requestRow.business_sector,
            requestRow.business_function,
            requestRow.operating_region,
            sSys,
            sRole,
            sPersona || requestRow.selected_persona,
            requestRow.access_duration,
            requestRow.justification,
            sApproverUser,
            'Approver',
            sStatus,
            sApproverComment
        ...
        // 5. If approved, update or insert role assignment in user_roles table
        if (sStatus === 'APPROVED') {
          ...
          if (requesterUser && roleRecord) {
            const isRevocation = (requestRow.access_type || "").toUpperCase() === 'REVOCATION' || (requestRow.business_function || "").toUpperCase().includes('REVOCATION');
            if (isRevocation) {
              await db.run(
                `UPDATE access_management.user_roles SET status = 'REVOKED' WHERE user_id = $1::uuid AND role_id = $2::uuid`,
                [requesterUser.id, roleRecord.id]
              );
            } else {
