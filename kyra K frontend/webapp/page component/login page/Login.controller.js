sap.ui.define([
    ...
    return Controller.extend("kyra001.pages.login.Login", {
        ...
        onLogin() {
            ...
            // Standard SAPUI5 OData V4 model action call
            const oAuthModel = this.getOwnerComponent().getModel("auth");
            const oOperation = oAuthModel.bindContext("/login(...)");
            oOperation.setParameter("username", sUserId);
            oOperation.setParameter("password", sPassword);
            oOperation.setParameter("role", sEffectiveTitle);
            ...
            oOperation.execute().then(() => {
                ...
                sessionStorage.setItem("kyra_active_user_uuid", oResult.userUuid);
                sessionStorage.setItem("kyra_active_role", sEffectiveTitle);

                // Set shared model properties immediately so the dashboard loads concurrently without delay
                ...
                if (sMessage.toLowerCase().includes("username")) {
                    oModel.setProperty("/idState", "Error");
                    oModel.setProperty("/idStateText", sMessage);
                } else if (sMessage.toLowerCase().includes("password")) {
                    oModel.setProperty("/passwordState", "Error");
